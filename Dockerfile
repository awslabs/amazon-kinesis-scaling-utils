...
WORKDIR /usr/local/kinesis_scaling
COPY --from=build-env /usr/local/kinesis_scaling/target/KinesisScalingUtils-.9.6.0-complete.jar ./KinesisScalingUtils-.9.6.0-complete.jar
COPY ./conf/configuration.json ./conf/
ENTRYPOINT [ \
    "java", \
    "-Dconfig-file-url=/usr/local/kinesis_scaling/conf/configuration.json", \
    "-cp", \
    "/usr/local/kinesis_scaling/KinesisScalingUtils-.9.6.0-complete.jar", \
